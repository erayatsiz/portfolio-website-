...
emailLinks.forEach(link => {
    link.addEventListener('click', (e) => {
        ...
        navigator.clipboard.writeText(email).then(() => {
            showNotification(`📧 E-posta adresi kopyalandı: ${email}`, 'success');
        }).catch(() => {
...
contactForm.addEventListener('submit', (e) => {
    ...
    const mailtoLink = `mailto:erayatsiz87@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(`İsim: ${name}\nE-posta: ${email}\n\nMesaj:\n${message}`)}`;

    // Try to open mailto
    window.location.href = mailtoLink;

    showNotification('✉️ E-posta uygulamanız açılıyor... (Eğer açılmazsa, e-posta adresine tıklayarak kopyalayabilirsiniz)', 'success');
